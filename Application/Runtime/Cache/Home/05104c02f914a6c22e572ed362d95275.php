      ...
      myNav[i].className="u-cursor active b-group-border1";   
            ...
            <div class="a-right">
              <div class="b-group-border2">
                ...
                <ul class="person">
                <form class="am-form tpl-form-line-form" enctype="multipart/form-data" method="post" action="<?php echo U('member/lecturer_edit');?>">
                <input type="hidden" name="lecturer_id" value="<?php echo ($data['lecturer_id']); ?>">
                <input type="hidden" name="table" value="lecturer">
                <input type="hidden" name="jump" value="lecturer">

                   <!--  <li> 
                   <p>注册号：<span><?php echo ($data['numbering']); ?></span></p>
                   <p>性别：<span><?php if($data['sex'] == 1): ?>男<?php else: ?>女<?php endif; ?></span></p>
                   <p>出生日期：<span><?php echo ($data['birth']); ?></span></p>
                   <p>身份证：<span><?php echo ($data['ids']); ?></span></p>
                   <p>所授课程：<span><?php echo ($data['subject']); ?></span></p>
                   <p>教师资质简介：<textarea style="background-color: #ffffff; border: 1px #D7D7D7 solid;" name="description" id="" ></textarea></p>
                   <p>注册时间：<?php echo (date('Y-m-d',$data["addtime"])); ?></p>
                   </li> -->
          ...
          <div class="td">
            <br>
            <p class="sec"><span class="blue">注册号</span><span><?php echo ($data['numbering']); ?></span></p>
            <p class="sec"><span class="blue">性别</span><span><?php if($data['sex'] == 1): ?>男<?php else: ?>女<?php endif; ?></span></p>
            
            <p class="sec"><span class="blue">邮箱</span><span><?php echo ($data['email']); ?></span></p> 
            <p class="sec"><span class="blue">电话</span><span><input type="text" name="tel" value="<?php echo ($data['tel']); ?>"/></span></p> 
            <p class="sec"><span class="blue">住址</span><span><input type="text" name="address" value="<?php echo ($data['address']); ?>"/></span></p> 
            <p class="sec"><span class="blue">出生日期</span><span><?php echo ($data['birth']); ?></span></p>   
            <p class="sec"><span class="blue">所授课程</span><span><?php echo ($data['subject']); ?></span></p>
            <p class="sec"><span class="blue">简介修改</span></p>
            <textarea style="background-color: #ffffff; border: 1px #D7D7D7 solid;width:815px;height:200px;" name="description" id=""><?php echo ($data['description']); ?></textarea>
            <p class="sec"><span class="blue">注册时间</span><span><?php echo (date('Y-m-d',$data["addtime"])); ?></span></p>
     
            
          </div>
        </div>
                <button style="width:200px;height:30px;background:#073763;border-radius:5px;margin:0 auto;display:block;"type="submit"><font color="#fff">修改</font> </button>               
                </form>
                </ul>
              </div>
            </div>
